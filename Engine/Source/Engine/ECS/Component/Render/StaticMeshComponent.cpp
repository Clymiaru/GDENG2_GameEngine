...
namespace Engine
{
	StaticMeshComponent::StaticMeshComponent(const EntityID& ownerID,
	...
	void StaticMeshComponent::SetTexture(const SharedPtr<TextureResource> texture)
	{
		m_TextureResource = texture;
	}

	void StaticMeshComponent::RemoveTexture()
	{
		m_TextureResource = nullptr;
	}

	void StaticMeshComponent::Draw(CameraComponent& camera)
	{
	...
	void StaticMeshComponent::Draw(EditorCameraComponent& camera)
	{
		RenderObjectData* constant = new RenderObjectData();
		constant->Model            = m_Transform->GetLocalMatrix();
		constant->ViewProjection   = camera.GetViewProjMatrix();
